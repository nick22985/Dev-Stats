...
export { getGithubStats, getContributedRepos, graphQLQuery};
...
function calculations(contribution) {
	let total = contribution.totalCommitContributions + contribution.totalIssueContributions +  contribution.totalPullRequestContributions
	+ contribution.totalPullRequestReviewContributions + contribution.totalRepositoriesWithContributedCommits + contribution.totalRepositoriesWithContributedIssues
	+ contribution.totalRepositoriesWithContributedPullRequestReviews + contribution.totalRepositoriesWithContributedPullRequests
	return total
}
...
function maxValueObject(obj, total) {
	...
	.toPairs()
	.value()
	let max = _.maxBy(tempObj)
	tempObj.map(pair => {
		pair.percent = (pair[1] / total ) * 100
		return pair
	});
	let newValue = {
		tempObj,
